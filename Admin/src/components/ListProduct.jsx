          ...
          <tbody>
            {allProducts.map((product,i)=>(
              <tr key={i} className="border-b border-slate-900/20 text-gray-20 p-6 medium-14">
                <td className="flexStart sm:flexBetween">
                  <img src={product.image} alt="" height={43} width={43} className="rounded-lg ring-1 ring-slate-900/5 my-1"></img>
                </td>
                <td><div className="line-clamp-3">{product.name}</div></td>
                <td>{product.old_price}</td>
                <td>{product.new_price}</td>
                <td>{product.category}</td>
                <td><div className="bold-22 pl-6 sm:pl-14"><TbTrash onClick={() => { remove_product(product.id);
                }} /></div></td>
              </tr>
